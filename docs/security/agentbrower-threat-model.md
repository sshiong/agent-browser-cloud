...
| TM-004 | Compromised runtime/provider | Code execution in runtime | Read Profile or proxy material | Credential exfiltration | Profile and secrets | Separate crates, loopback CDP, no secrets in Session Context | OS-level helper split not fully enforced | Separate processes, seccomp, Landlock and dedicated UIDs | helper syscall/IPC audit | medium | high | high |
...
| TM-007 | Operator/admin | Admin token | Misuse break-glass or retention controls | Evidence/data compromise | Audit and Profiles | Admin MFA validator and append-only hash chain | Break-glass API/process incomplete | Time-bound approval, reason and immutable audit | admin action alerts | low | high | medium |
