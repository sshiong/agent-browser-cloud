...
for _ in $(seq 1 160); do
  tab_resource_limits="$(docker exec "$postgres_name" psql -U browsercloud -d browsercloud -Atc \
    "select state_collector_budget_percent || ':' || background_tabs_frozen || ':' || new_tabs_blocked || ':' || paused_extension_ids::text || ':' || success_trace_sample_percent || ':' || observer_frame_rate_fps
     from browser_placements where session_id='${session_one}'")"
  if [[ "$tab_resource_limits" = '50:true:true:["jdgnleokimdbblcflcfcohbinohmmmlb"]:10:0' ]]; then break; fi
  sleep 0.25
done
test "$tab_resource_limits" = '50:true:true:["jdgnleokimdbblcflcfcohbinohmmmlb"]:10:0'
...
printf '%s' "$tab_resource_view" | python3 -c \
  'import json,sys; allocation=json.load(sys.stdin)["allocation"]; assert allocation["backgroundTabsFrozen"] is True; assert allocation["newTabsBlocked"] is True; assert allocation["pausedExtensionIds"] == ["jdgnleokimdbblcflcfcohbinohmmmlb"]; assert allocation["stateCollectorBudgetPercent"] == 50; assert allocation["successTraceSamplePercent"] == 10; assert allocation["observerFrameRateFps"] == 0'
...
non_cgroup_resource_limits="$(docker exec "$postgres_name" psql -U browsercloud -d browsercloud -Atc \
  "select state_collector_budget_percent || ':' || remote_desktop_bitrate_kbps || ':' || extension_cpu_weight || ':' || media_encoder_slots || ':' || background_tabs_frozen || ':' || new_tabs_blocked || ':' || success_trace_sample_percent || ':' || observer_frame_rate_fps
   from browser_placements where session_id='${session_one}'")"
test "$non_cgroup_resource_limits" = "50:0:100:0:true:true:10:0"
...
for _ in $(seq 1 40); do
  ...
  if [[ "$resource_cost_snapshot" = local-l2-v1:* ]]; then break; fi
  sleep 0.25
done
test "${resource_cost_snapshot%%:*}" = "local-l2-v1"
...
printf '%s' "$session_resources_with_cost" | python3 -c \
  'import json,sys; item=json.load(sys.stdin); cost=item["cost"]; assert cost["pricingVersion"] == "local-l2-v1"; assert float(cost["currentHourlyCost"]) > 0; assert len(cost["trend"]) >= 1; assert cost["trend"][-1]["pricingVersion"] == "local-l2-v1"'
