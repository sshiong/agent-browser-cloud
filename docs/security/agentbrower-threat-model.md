...
| TM-004 | Compromised runtime/provider | Code execution in runtime | Read Profile or proxy material | Credential exfiltration | Profile and secrets | Network/Storage Helper independent processes, fixed bounded IPC, Peer UID, dedicated container UIDs/seccomp/capability drop, separate Profile volume; loopback CDP; no secrets in Session Context | GPU/LSM profiles, unified helper audit and real-cluster cross-UID evidence pending | Enforce Landlock/AppArmor/SELinux, dedicated audit identities and cluster GameDay | helper syscall/IPC audit | medium | high | high |
...
| TM-006 | Tenant/traffic burst | API access | Exhaust CP/Node/desktop resources | Multi-tenant outage | Availability | Bounded messages, resource classes, deadlines, signed per-Actor desktop output quota | Workspace quota overrides and shared API admission facts not complete | Tenant cost units and emergency lanes | saturation/error-budget alerts | high | medium | high |
| TM-007 | Operator/admin | Admin token | Misuse break-glass or retention controls | Evidence/data compromise | Audit and Profiles | Admin MFA, dual-control time-bound break-glass, single-use/15-minute/single-operator Secure Debug, purpose-limited projection, per-access evidence chain, revocation checks and tenant audit hash chain | Independent Debug Worker, pixel recording/WORM and retention deletion receipt incomplete | Isolated worker, signed recording manifest and retention evidence | admin action alerts | low | high | medium |
