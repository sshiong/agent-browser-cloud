...
| 故障矩阵             | 已覆盖 Browser/Node、PostgreSQL 短时不可用、Proxy、Profile Corruption、Key-up、Diff、Workflow DLQ                                          | 补 Object Storage 超时、Coordinator Kill/接管和自动 GameDay                       |
...
`audit_chain_valid=true` 和 50 条审计事件。
