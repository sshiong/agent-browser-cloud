...
for _ in $(seq 1 160); do
  tab_resource_limits="$(docker exec "$postgres_name" psql -U browsercloud -d browsercloud -Atc \
    "select state_collector_budget_percent || ':' || background_tabs_frozen || ':' || new_tabs_blocked
     from browser_placements where session_id='${session_one}'")"
  if [[ "$tab_resource_limits" = "50:true:true" ]]; then break; fi
  sleep 0.25
done
test "$tab_resource_limits" = "50:true:true"
...
printf '%s' "$tab_resource_view" | python3 -c \
  'import json,sys; allocation=json.load(sys.stdin)["allocation"]; assert allocation["backgroundTabsFrozen"] is True; assert allocation["newTabsBlocked"] is True; assert allocation["stateCollectorBudgetPercent"] == 50'
