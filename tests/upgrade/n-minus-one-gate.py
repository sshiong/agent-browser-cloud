...
browser_node_freshness_migration = read(
    "database/migrations/V101__browser_node_freshness_projection.sql"
)
browser_node_freshness_upper = browser_node_freshness_migration.upper()
for invariant in (
    ...
    assert invariant in browser_node_freshness_upper, (
        f"Browser Node freshness migration lacks rolling invariant: {invariant}"
    )
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in browser_node_freshness_upper, (
        f"Browser Node freshness migration contains incompatible operation: {forbidden}"
    )

enterprise_overview_stream_migration = read(
    "database/migrations/V102__enterprise_overview_event_stream.sql"
)
enterprise_overview_stream_upper = enterprise_overview_stream_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN"):
    assert forbidden not in enterprise_overview_stream_upper, (
        f"Enterprise Overview stream migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    "APPEND_ENTERPRISE_OVERVIEW_MEDIA_PLACEMENT_UPDATE",
    "SCHEDULE_ENTERPRISE_OVERVIEW_ERROR_BUDGET_EXPIRY",
    "SCHEDULE_ENTERPRISE_OVERVIEW_GAMEDAY_TREND_EXPIRY",
    "APPEND_ENTERPRISE_OVERVIEW_COMPLIANCE",
):
    assert invariant in enterprise_overview_stream_upper, (
        f"Enterprise Overview stream migration lacks complete additive invariant: {invariant}"
    )

challenge_visual_migration = read(
    "database/migrations/V103__challenge_visual_automation.sql"
)
challenge_visual_upper = challenge_visual_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in challenge_visual_upper, (
        f"Challenge visual automation migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    "VALIDATE CONSTRAINT CHK_SESSIONS_CHALLENGE_AUTOMATION_ATTEMPTS",
):
    assert invariant in challenge_visual_upper, (
        f"Challenge visual automation migration lacks additive invariant: {invariant}"
    )

node_contract = read("packages/contracts/proto/node/v1/node_command.proto")
for invariant in (
    "message ChallengeVisualAction",
    "message ChallengeAutomationActionCommand",
    "message ChallengeAutomationFailedEvent",
):
    assert invariant in node_contract, f"Challenge visual Node contract missing: {invariant}"

agent_autonomous_migration = read(
    "database/migrations/V104__agent_autonomous_sensitive_inputs.sql"
)
agent_autonomous_upper = agent_autonomous_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in agent_autonomous_upper, (
        f"Agent autonomous input migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    "VALIDATE CONSTRAINT CHK_SESSIONS_AGENT_CONTROL_MODE",
):
    assert invariant in agent_autonomous_upper, (
        f"Agent autonomous input migration lacks additive invariant: {invariant}"
    )
agent_action_contract = node_contract.split("message AgentActionCommand {", 1)[1].split("}", 1)[0]
for field_name, field_tag in (("allow_sensitive_target", 14), ("maximum_attempts", 15)):
...
agent_human_input_migration = read(
    "database/migrations/V105__agent_human_input_assistance.sql"
)
agent_human_input_upper = agent_human_input_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in agent_human_input_upper, (
        f"Agent human-input assistance migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    "VALIDATE CONSTRAINT CHK_CHALLENGE_EVENT_TARGET_V2",
    "'AGENT_HUMAN_INPUT'",
    "CREATE TRIGGER APPEND_AGENT_CHALLENGE_INPUT_ENVELOPE",
    "FOREIGN KEY (SECRET_ID) REFERENCES AGENT_INPUT_SECRETS(SECRET_ID)",
):
    assert invariant in agent_human_input_upper, (
        f"Agent human-input assistance migration lacks additive invariant: {invariant}"
    )

challenge_motion_migration = read(
    "database/migrations/V106__challenge_human_motion_policy.sql"
)
challenge_motion_upper = challenge_motion_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in challenge_motion_upper, (
        f"Challenge motion migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    "VALIDATE CONSTRAINT CHK_SESSIONS_CHALLENGE_MOTION_STEPS",
    "ADD COLUMN MOTION_MIN_STEPS SMALLINT NOT NULL DEFAULT 8",
    "VALIDATE CONSTRAINT CHK_CHALLENGE_RUN_MOTION_DELAY",
):
    assert invariant in challenge_motion_upper, (
        f"Challenge motion migration lacks additive invariant: {invariant}"
    )

identity_lock_migration = read("database/migrations/V107__session_identity_spec_lock.sql")
identity_lock_upper = identity_lock_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in identity_lock_upper, (
        f"Session identity migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    assert invariant in identity_lock_upper, (
        f"Session identity migration lacks additive invariant: {invariant}"
    )

agent_clipboard_migration = read("database/migrations/V108__agent_clipboard.sql")
agent_clipboard_upper = agent_clipboard_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in agent_clipboard_upper, (
        f"Agent clipboard migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    assert invariant in agent_clipboard_upper, (
...
agent_browser_screenshot_migration = read(
    "database/migrations/V110__agent_browser_screenshots.sql"
)
agent_browser_screenshot_upper = agent_browser_screenshot_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in agent_browser_screenshot_upper, (
        f"Agent Browser screenshot migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    "AGENT_PERCEPTION",
    "CAPTURE_MODE IN ('VIEWPORT', 'FULL_PAGE', 'ELEMENT', 'REGION', 'CHALLENGE_REGION')",
    "PIXELS, OBJECT STORAGE PATHS AND SIGNED URLS ARE NEVER PERSISTED",
):
    assert invariant in agent_browser_screenshot_upper, (
        f"Agent Browser screenshot migration lacks additive invariant: {invariant}"
    )

agent_browser_evaluation_migration = read(
    "database/migrations/V111__agent_browser_javascript_evaluations.sql"
)
agent_browser_evaluation_upper = agent_browser_evaluation_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in agent_browser_evaluation_upper, (
        f"Agent Browser evaluation migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    "EVALUATION_MODE IN ('READ_ONLY', 'PAGE_ACTION')",
    "STATE IN ('EXECUTING', 'COMMITTED', 'FAILED')",
    "SCRIPT SOURCE IS SEALED IN OUTBOX",
):
    assert invariant in agent_browser_evaluation_upper, (
        f"Agent Browser evaluation migration lacks additive invariant: {invariant}"
    )

agent_clipboard_bridge_migration = read(
    "database/migrations/V112__agent_clipboard_bridge.sql"
)
agent_clipboard_bridge_upper = agent_clipboard_bridge_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in agent_clipboard_bridge_upper, (
        f"Agent clipboard bridge migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    "PURPOSE IN ('OPERATOR_COPY', 'AUTOMATION_HANDOFF', 'HUMAN_ASSISTANCE')",
    "STATE IN ('ISSUED', 'COMPLETED', 'EXPIRED')",
    "CONTENT IS NEVER PERSISTED",
):
    assert invariant in agent_clipboard_bridge_upper, (
        f"Agent clipboard bridge migration lacks additive invariant: {invariant}"
    )

session_soft_delete_migration = read(
    "database/migrations/V113__session_soft_deletion.sql"
)
session_soft_delete_upper = session_soft_delete_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in session_soft_delete_upper, (
        f"Session soft deletion migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    "ADD COLUMN DELETED_AT TIMESTAMPTZ",
    "ADD COLUMN DELETED_BY TEXT",
    "ADD COLUMN DELETION_BATCH_ID TEXT",
    "CHK_SESSION_SOFT_DELETE_FIELDS",
    "VALIDATE CONSTRAINT CHK_SESSION_SOFT_DELETE_FIELDS",
    "WHERE DELETED_AT IS NULL",
):
    assert invariant in session_soft_delete_upper, (
        f"Session soft deletion migration lacks additive invariant: {invariant}"
    )

browser_state_observed_at_migration = read(
    "database/migrations/V114__browser_state_observed_at.sql"
)
browser_state_observed_at_upper = browser_state_observed_at_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in browser_state_observed_at_upper, (
        f"Browser State observed-at migration contains incompatible operation: {forbidden}"
    )
assert "ADD COLUMN OBSERVED_AT TIMESTAMPTZ" in browser_state_observed_at_upper
for field_name, field_tag in (
    ...
    observed_contract = node_contract.split(
        "message BrowserStateObservedEvent {", 1
    )[1].split("}", 1)[0]
    assert re.search(rf"\b{field_name}\s*=\s*{field_tag}\s*;", observed_contract), (
        f"Browser State observation lacks additive {field_name} tag {field_tag}"
    )
assert "NEW.state_json = OLD.state_json" in browser_state_observed_at_migration

agent_action_attempt_migration = read(
    "database/migrations/V115__agent_action_attempt_history.sql"
)
agent_action_attempt_upper = agent_action_attempt_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in agent_action_attempt_upper, (
        f"Agent action-attempt migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    assert invariant in agent_action_attempt_upper, (
        f"Agent action-attempt migration lacks additive invariant: {invariant}"
    )
agent_action_attempt_service = read(
    "apps/control-plane/src/main/java/io/browsercloud/application/AgentActionAttemptService.java"
)
for invariant in (
    "AGENT_ACTION_LOOP_DETECTED",
    "MAXIMUM_CONSECUTIVE_ATTEMPTS = 2",
    "step.toolId() == ToolId.WAIT_FOR",
):
    assert invariant in agent_action_attempt_service, (
        f"Agent action loop detection lacks invariant: {invariant}"
    )

agent_task_memory_migration = read(
    "database/migrations/V116__agent_task_memory_history.sql"
)
agent_task_memory_upper = agent_task_memory_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN", "DROP TABLE"):
    assert forbidden not in agent_task_memory_upper, (
        f"Agent task-memory migration contains incompatible operation: {forbidden}"
    )
for invariant in (
    ...
    assert invariant in agent_task_memory_upper, (
        f"Agent task-memory migration lacks additive invariant: {invariant}"
    )
agent_task_memory_service = read(
    "apps/control-plane/src/main/java/io/browsercloud/application/AgentTaskMemoryService.java"
)
for invariant in (
    "ON CONFLICT (task_id, event_key) DO NOTHING",
    "MAXIMUM_VISIBLE_EVENTS = 100",
    "actionAttempts.descriptorHash(step)",
):
    assert invariant in agent_task_memory_service, (
        f"Agent task-memory service lacks invariant: {invariant}"
    )
agent_task_memory_openapi = read("packages/contracts/openapi/session-api.yaml")
agent_task_memory_schema = agent_task_memory_openapi.split("    AgentTaskMemory:", 1)[1].split(
    "    ClaimAgentExecutionJobRequest:", 1
)[0]
for invariant in ("executionHistory:", "AgentExecutionMemoryEvent", "maxItems: 100"):
    assert invariant in agent_task_memory_schema, (
        f"Agent task-memory API lacks rolling-compatible invariant: {invariant}"
    )
agent_task_memory_parent_schema = agent_task_memory_openapi.split("    AgentTask:", 1)[1].split(
    "    AgentRecoveryGuidance:", 1
)[0]
assert "        - memory" not in agent_task_memory_parent_schema, (
    "AgentTask.memory must stay optional during N/N-1 rolling upgrades"
)
...
for field_name, field_tag in (("tab_id", 18), ("tab_url", 19)):
    assert re.search(rf"\b{field_name}\s*=\s*{field_tag}\s*;", agent_action_contract), (
        f"AgentActionCommand lacks additive {field_name} tag {field_tag}"
    )
...
for field_name, field_tag in (("tab_id", 13), ("tab_url", 14)):
    assert re.search(
        rf"\b{field_name}\s*=\s*{field_tag}\s*;", agent_action_primitive_contract
    ), f"AgentActionPrimitive lacks additive {field_name} tag {field_tag}"
...
for field_name, field_tag in (
    ...
    ("identity_timezone", 40),
    ("identity_locale", 41),
    ("identity_languages", 42),
    ("identity_webrtc_policy", 43),
    ("identity_dns_policy", 44),
    ("identity_viewport_width", 45),
    ("identity_viewport_height", 46),
    ("identity_screen_width", 47),
    ("identity_screen_height", 48),
    ("identity_device_scale_factor", 49),
    ("identity_fingerprint_profile", 50),
    ("identity_operating_system_profile", 51),
    ("identity_spec_version", 52),
    ("identity_spec_hash", 53),
):
    assert re.search(rf"\b{field_name}\s*=\s*{field_tag}\s*;", start_runtime_contract), (
        f"StartRuntimeCommand lacks additive identity field {field_name} tag {field_tag}"
    )
challenge_action_contract = node_contract.split(
    "message ChallengeAutomationActionCommand {", 1
)[1].split("}", 1)[0]
for field_name, field_tag in (
    ("motion_min_steps", 9),
    ("motion_max_steps", 10),
    ("motion_min_delay_ms", 11),
    ("motion_max_delay_ms", 12),
    ("target_offset_ratio", 13),
    ("target_ref", 14),
    ("target_revision", 15),
    ("expected_x", 16),
    ("expected_y", 17),
    ("expected_width", 18),
    ("expected_height", 19),
    ("visual_anchor_hash", 20),
    ("opaque_frame_ref", 21),
    ("opaque_frame_x", 22),
    ("opaque_frame_y", 23),
    ("opaque_frame_width", 24),
    ("opaque_frame_height", 25),
):
    assert re.search(rf"\b{field_name}\s*=\s*{field_tag}\s*;", challenge_action_contract), (
        f"ChallengeAutomationActionCommand lacks additive {field_name} tag {field_tag}"
    )
challenge_automation_service = read(
    "apps/control-plane/src/main/java/io/browsercloud/application/ChallengeAutomationApplicationService.java"
)
for invariant in (
    ...
    assert invariant in recording_node_agent, (
        f"Opaque frame Challenge Node capability is missing: {invariant}"
    )
for invariant in ('"opaqueFrameChallengeClick"', '"state-fenced-click-v1"'):
    assert invariant in challenge_automation_service, (
        f"Opaque frame Challenge Control Plane capability gate is missing: {invariant}"
    )
...
assert evidence_event_tags == {
    ...
    "capture_mode": 16,
    "captured_state_version": 17,
    "captured_target_revision": 18,
    "captured_state_hash": 19,
    "captured_active_tab_id": 20,
    "viewport_width": 21,
    "viewport_height": 22,
    "device_scale_factor": 23,
    "captured_region_x": 24,
    "captured_region_y": 25,
    "captured_region_width": 26,
    "captured_region_height": 27,
    "coordinate_space": 28,
}
...
for message_name in ("BrowserStateEvent", "BrowserStateDiffEvent"):
    ...
    tab_tags = (
        (("tabs", 15), ("active_tab_id", 16))
        if message_name == "BrowserStateEvent"
        else (("tabs", 18), ("active_tab_id", 19))
    )
    for field, tag in tab_tags:
        assert re.search(rf"\b{field}\s*=\s*{tag};", state_message), (
            f"{message_name} must keep additive Browser tab tag {tag} for {field}"
        )
    dialog_tags = (
        (("native_dialogs", 17), ("native_dialog_evidence_fresh", 18))
        if message_name == "BrowserStateEvent"
        else (("native_dialogs", 20), ("native_dialog_evidence_fresh", 21))
    )
    for field, tag in dialog_tags:
        assert re.search(rf"\b{field}\s*=\s*{tag};", state_message), (
...
for field, tag in (("tab_id", 1), ("url", 2), ("title", 3), ("active", 4)):
    assert re.search(rf"\b{field}\s*=\s*{tag};", browser_tab_state)

browser_native_dialog_state = proto.split(
    "message BrowserNativeDialogState {", 1
)[1].split("}", 1)[0]
for field, tag in (
    ("dialog_id", 1),
    ("tab_id", 2),
...
session_evidence_event = proto.split(
    "message SessionEvidenceCapturedEvent {", 1
)[1].split("}", 1)[0]
for field, tag in (
    ("capture_mode", 16),
    ("captured_state_version", 17),
    ...
    assert re.search(rf"\b{field}\s*=\s*{tag};", session_evidence_event), (
        f"SessionEvidenceCapturedEvent lacks additive Agent screenshot {field} tag {tag}"
    )
for invariant in (
    ...
    assert invariant in recording_capacity_service or invariant in recording_node_agent, (
        f"Agent screenshot rolling capability gate is missing: {invariant}"
    )

agent_evaluation_command = proto.split(
    "message AgentBrowserEvaluateCommand {", 1
)[1].split("}", 1)[0]
for field, tag in (
    ("session_id", 1),
    ("evaluation_id", 2),
    ...
    ("await_promise", 10),
    ("timeout_ms", 11),
    ("maximum_result_bytes", 12),
):
    assert re.search(rf"\b{field}\s*=\s*{tag};", agent_evaluation_command), (
        f"AgentBrowserEvaluateCommand lacks additive {field} tag {tag}"
    )
agent_evaluation_event = proto.split(
    "message AgentBrowserEvaluationCompletedEvent {", 1
)[1].split("}", 1)[0]
for field, tag in (
    ("session_id", 1),
    ("evaluation_id", 2),
    ...
    ("result_json", 5),
    ("result_bytes", 6),
    ("redacted_value_count", 7),
    ("exception_class", 8),
    ("exception_message", 9),
    ("error_code", 10),
    ("state_version_before", 11),
    ("target_revision_before", 12),
    ("state_hash_before", 13),
    ("active_tab_id_before", 14),
    ("state_version_after", 15),
    ("target_revision_after", 16),
    ("state_hash_after", 17),
    ("active_tab_id_after", 18),
    ("duration_ms", 19),
):
    assert re.search(rf"\b{field}\s*=\s*{tag};", agent_evaluation_event), (
        f"AgentBrowserEvaluationCompletedEvent lacks additive {field} tag {tag}"
    )
for invariant in (
    '"agentJavascriptEvaluate".to_owned()',
    '"state-fenced-bounded-v1"',
    '"AgentBrowserEvaluate"',
):
    assert invariant in recording_capacity_service or invariant in recording_node_agent, (
        f"Agent Browser evaluation rolling capability gate is missing: {invariant}"
    )
...
# Additive SSE endpoint: an N-1 console keeps polling the unchanged Overview operation.
for enterprise_overview_stream_contract in (
    "/api/v1/enterprise/overview/event-stream:",
    "operationId: streamEnterpriseOverviewChanges",
    "EnterpriseOverviewStreamControl:",
    "EnterpriseOverviewStreamChange:",
):
    assert enterprise_overview_stream_contract in openapi
...
assert "BROWSER_NODE_FRESHNESS_THRESHOLD_MS" in workloads
assert "BROWSER_NODE_FRESHNESS_PROJECTION_INTERVAL_MS" in workloads
...
proxy_learning_migration = read(
    "database/migrations/V129__proxy_business_outcome_learning.sql"
)
proxy_learning_upper = proxy_learning_migration.upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN"):
    assert forbidden not in proxy_learning_upper, (
        f"Proxy outcome learning migration is not rolling additive: {forbidden}"
    )
for invariant in (
    ...
    "ADD COLUMN SELECTION_REASON",
    "NOT VALID",
    "VALIDATE CONSTRAINT CHK_SESSION_PROXY_BINDING_SELECTION_REASON",
):
    assert invariant in proxy_learning_upper, (
        f"Proxy outcome learning migration lacks invariant: {invariant}"
    )
proxy_candidate_schema = openapi.split("    ProxyRoutingCandidateScore:", 1)[1].split(
    "    ProxyRoutingDecision:", 1
)[0]
proxy_candidate_required = proxy_candidate_schema.split("      properties:", 1)[0]
for additive_field in (
    "businessOutcomeScore",
    "businessOutcomeSampleCount",
    "profileSticky",
    "explorationEligible",
):
    assert additive_field not in proxy_candidate_required, (
        f"Proxy learning field must remain optional for N-1 servers: {additive_field}"
    )
proxy_stickiness_index = read(
    "database/migrations/V130__proxy_route_profile_stickiness_index.sql"
).upper()
assert "CREATE INDEX CONCURRENTLY" in proxy_stickiness_index
assert "ON SESSIONS(TENANT_ID, PROFILE_ID, CREATED_AT DESC)" in proxy_stickiness_index
assert "EXECUTEINTRANSACTION=FALSE" in read(
    "database/migrations/V130__proxy_route_profile_stickiness_index.sql.conf"
).upper()

proxy_site_challenge_migration = read(
    "database/migrations/V131__proxy_site_challenge_quarantine.sql"
).upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN"):
    assert forbidden not in proxy_site_challenge_migration, (
        f"Proxy site Challenge migration is not rolling additive: {forbidden}"
    )
for invariant in (
    "CREATE TABLE PROXY_ROUTE_SITE_CHALLENGES",
    "SITE_DOMAIN_HASH",
    "ALTER TABLE SESSION_PROXY_BINDING_ASSIGNMENTS",
    "ADD COLUMN ROUTING_SITE_DOMAIN_HASH",
    "NOT VALID",
    "VALIDATE CONSTRAINT CHK_SESSION_PROXY_BINDING_ROUTING_SITE_HASH",
):
    assert invariant in proxy_site_challenge_migration, (
        f"Proxy site Challenge migration lacks invariant: {invariant}"
    )

proxy_provider_adapter_migration = read(
    "database/migrations/V132__proxy_provider_adapter_endpoint_identity.sql"
).upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN"):
    assert forbidden not in proxy_provider_adapter_migration, (
        f"Proxy Provider Adapter migration is not rolling additive: {forbidden}"
    )
for invariant in (
    "ADD COLUMN PROVIDER_ENDPOINT_ID TEXT",
    "ADD COLUMN PROVIDER_ADAPTER_TYPE TEXT DEFAULT 'CONFIGURED_HTTP'",
    "PROVIDER_ENDPOINT_ID IS NULL OR LENGTH(PROVIDER_ENDPOINT_ID) BETWEEN 1 AND 256",
    "PROVIDER_ADAPTER_TYPE IS NULL OR PROVIDER_ADAPTER_TYPE IN ('CONFIGURED_HTTP')",
    ...
    assert invariant in proxy_provider_adapter_migration, (
        f"Proxy Provider Adapter migration lacks rolling invariant: {invariant}"
    )
assert "ALTER COLUMN PROVIDER_ENDPOINT_ID SET NOT NULL" not in proxy_provider_adapter_migration
assert "UPDATE PROXY_ALLOCATIONS" not in proxy_provider_adapter_migration
remote_proxy_provider_adapter_migration = read(
    "database/migrations/V133__remote_proxy_provider_adapter_type.sql"
).upper()
for forbidden in ("DROP COLUMN", "RENAME COLUMN", "ALTER COLUMN"):
    assert forbidden not in remote_proxy_provider_adapter_migration, (
        f"Remote Proxy Provider Adapter migration is not rolling compatible: {forbidden}"
    )
for invariant in (
    "DROP CONSTRAINT PROXY_ALLOCATIONS_PROVIDER_ADAPTER_TYPE_CHECK",
    "PROVIDER_ADAPTER_TYPE IN ('CONFIGURED_HTTP', 'REMOTE_HTTP_V1')",
    "NOT VALID",
    "VALIDATE CONSTRAINT PROXY_ALLOCATIONS_PROVIDER_ADAPTER_TYPE_CHECK",
):
    assert invariant in remote_proxy_provider_adapter_migration, (
        f"Remote Proxy Provider Adapter migration lacks invariant: {invariant}"
    )
assert "UPDATE PROXY_ALLOCATIONS" not in remote_proxy_provider_adapter_migration
create_session_schema = openapi.split("    CreateSessionRequest:", 1)[1].split(
    "    CreateSessionResponse:", 1
)[0]
create_session_required = create_session_schema.split("      properties:", 1)[0]
assert "proxyRoutingDomain" not in create_session_required, (
    "Proxy site-routing hint must remain optional for N-1 clients"
)
...
vision_worker_deployment = read(
    "deploy/kubernetes/base/vision-worker-deployment.yaml"
)
for invariant in (
    "runtimeClassName: agent-sandbox",
    "automountServiceAccountToken: false",
    ...
    "allowPrivilegeEscalation: false",
    "drop: [ALL]",
    'command: ["python", "/app/vision_worker.py"]',
    "--allowed-screenshot-host=$(EVIDENCE_DOWNLOAD_ALLOWED_HOST)",
    "--environment=production",
):
    assert invariant in vision_worker_deployment
for invariant in (
    "name: vision-worker-default-deny",
    "name: vision-worker-controlled-egress",
    'browsercloud.io/evidence-download: "true"',
):
    assert invariant in validation_worker_network_policy
for invariant in (
    "/api/v1/challenge-visual-jobs:claim:",
    "operationId: completeChallengeVisualJob",
    "ClaimChallengeVisualJobRequest:",
    "ChallengeVisualJobClaim:",
    "const: challenge-vision-worker/v1",
    "local-ocr-pii-gate-v1",
    "privacyScanVersion:",
    "ocrTextHash:",
    "detectedSensitivePatternCount:",
    "piiRedactedRegionCount:",
    "remainingSensitivePatternCount:",
):
    assert invariant in openapi

vision_privacy_migration = read(
    "database/migrations/V119__challenge_visual_pixel_privacy_attestation.sql"
)
vision_privacy_upper = vision_privacy_migration.upper()
for invariant in (
    "ADD COLUMN PRIVACY_SCAN_VERSION",
    "ADD COLUMN OCR_TEXT_HASH",
    "ADD COLUMN DETECTED_SENSITIVE_PATTERN_COUNT",
    "ADD COLUMN PII_REDACTED_REGION_COUNT",
    "ADD COLUMN REMAINING_SENSITIVE_PATTERN_COUNT",
    "ADD COLUMN CAPTURE_REGION_WIDTH",
    "ADD COLUMN CAPTURED_STATE_HASH",
    "CHALLENGE_SCREENSHOT",
    "CHK_SESSION_EVIDENCE_KIND_V4",
    "TESSERACT-PII-V1",
    "REMAINING_SENSITIVE_PATTERN_COUNT = 0",
    "NOT VALID",
    "VALIDATE CONSTRAINT",
):
    assert invariant in vision_privacy_upper
for forbidden in (" OCR_TEXT TEXT", " SCREENSHOT_BYTES ", " RAW_OCR "):
    assert forbidden not in vision_privacy_upper

worker_queue_notification_migration = read(
    "database/migrations/V120__worker_queue_notifications.sql"
)
worker_queue_notification_upper = worker_queue_notification_migration.upper()
for invariant in (
    "PG_NOTIFY('BROWSERCLOUD_WORKER_JOBS'",
    ...
    assert invariant in worker_queue_notification_upper
for forbidden in ("ALTER TABLE", "DROP TABLE", "DELETE FROM", "UPDATE AGENT_"):
    assert forbidden not in worker_queue_notification_upper

idempotency_response_migration = read(
    "database/migrations/V121__api_idempotency_response_snapshot.sql"
)
idempotency_response_upper = idempotency_response_migration.upper()
for invariant in (
    "ALTER TABLE API_IDEMPOTENCY_RECORDS",
    "ADD COLUMN RESPONSE_PAYLOAD TEXT",
):
    assert invariant in idempotency_response_upper
for forbidden in ("NOT NULL", "DROP TABLE", "DELETE FROM", "UPDATE API_IDEMPOTENCY_RECORDS"):
    assert forbidden not in idempotency_response_upper

challenge_structural_migration = read(
    "database/migrations/V122__challenge_structural_single_click.sql"
)
challenge_structural_upper = challenge_structural_migration.upper()
for invariant in (
    ...
    assert invariant in challenge_structural_upper
for forbidden in ("DROP TABLE", "DROP COLUMN", "DELETE FROM", "UPDATE CHALLENGE_VISUAL_JOBS"):
    assert forbidden not in challenge_structural_upper

profile_session_health_migration = read(
    "database/migrations/V123__profile_site_session_health.sql"
)
profile_session_health_upper = profile_session_health_migration.upper()
for invariant in (
    ...
    assert invariant in profile_session_health_upper
for forbidden in ("ALTER TABLE", "DROP TABLE", "DROP COLUMN", "DELETE FROM", "UPDATE PROFILES"):
    assert forbidden not in profile_session_health_upper

outcome_semantic_evidence_migration = read(
    "database/migrations/V124__agent_outcome_semantic_evidence.sql"
)
outcome_semantic_evidence_upper = outcome_semantic_evidence_migration.upper()
for invariant in (
    "ADD COLUMN SEMANTIC_EVIDENCE_HASH TEXT",
    "SEMANTIC_EVIDENCE_HASH IS NULL OR SEMANTIC_EVIDENCE_HASH ~",
    "NOT VALID",
    "VALIDATE CONSTRAINT CHK_AGENT_OUTCOME_SEMANTIC_EVIDENCE_HASH",
):
    assert invariant in outcome_semantic_evidence_upper
for forbidden in (
    "DROP TABLE",
    "DROP COLUMN",
    "DELETE FROM",
    "UPDATE AGENT_OUTCOME_VERIFICATION_JOBS",
    "ALTER COLUMN SEMANTIC_EVIDENCE_HASH SET NOT NULL",
):
    assert forbidden not in outcome_semantic_evidence_upper
assert 'job.semanticEvidenceHash() == null' in read(
    "apps/control-plane/src/main/java/io/browsercloud/application/AgentOutcomeVerifierApplicationService.java"
), "new Control Plane must preserve exact fencing for N-1 jobs without semantic evidence"

opaque_frame_click_migration = read(
    "database/migrations/V125__opaque_frame_challenge_click_policy.sql"
)
opaque_frame_click_upper = opaque_frame_click_migration.upper()
for invariant in (
    ...
    "VALIDATE CONSTRAINT CHK_CHALLENGE_EVENT_TYPE_V2",
    "DROP CONSTRAINT CHK_CHALLENGE_EVENT_TYPE",
    "VALIDATE CONSTRAINT CHK_CHALLENGE_EVENT_TARGET_V3",
):
    assert invariant in opaque_frame_click_upper
for forbidden in ("DROP TABLE", "DROP COLUMN", "DELETE FROM", "UPDATE SESSIONS"):
    assert forbidden not in opaque_frame_click_upper

otp_target_fence_migration = read(
    "database/migrations/V126__otp_target_challenge_fence.sql"
)
otp_target_fence_upper = otp_target_fence_migration.upper()
for invariant in (
    "ADD CONSTRAINT CHK_CHALLENGE_EVENT_TARGET_V4",
    "SUSPECTED_TYPE = 'OTP'",
    "TARGET_REF IS NULL AND VISUAL_ANCHOR_HASH IS NULL",
    "TARGET_REF IS NOT NULL AND VISUAL_ANCHOR_HASH ~",
    "NOT VALID",
    "VALIDATE CONSTRAINT CHK_CHALLENGE_EVENT_TARGET_V4",
    "DROP CONSTRAINT CHK_CHALLENGE_EVENT_TARGET",
):
    assert invariant in otp_target_fence_upper
for forbidden in ("DROP TABLE", "DROP COLUMN", "DELETE FROM", "UPDATE CHALLENGE_EVENTS"):
    assert forbidden not in otp_target_fence_upper

recording_playback_migration = read(
    "database/migrations/V127__recording_playback_access_grants.sql"
)
recording_playback_upper = recording_playback_migration.upper()
for invariant in (
    ...
    assert invariant in recording_playback_upper
for forbidden in (
    "DROP TABLE",
    "DROP COLUMN",
    "DELETE FROM",
    ...
    assert forbidden not in recording_playback_upper
recording_playback_service = read(
    "apps/control-plane/src/main/java/io/browsercloud/application/SessionRecordingPlaybackApplicationService.java"
)
for invariant in (
    'nodeHasCapability(claim.nodeId(), "recordingPlayback", "presigned-segments-v1")',
    "SIGNED_URL_SECONDS = 60",
    "SEGMENT_PAGE_SIZE = 24",
):
    assert invariant in recording_playback_service
for invariant in ('"recordingPlayback".to_owned()', '"presigned-segments-v1"'):
    assert invariant in recording_node_agent

recording_deletion_migration = read(
    "database/migrations/V128__recording_retention_physical_deletion.sql"
)
recording_deletion_upper = recording_deletion_migration.upper()
for invariant in (
    "ADD COLUMN DELETED_AT TIMESTAMPTZ",
    ...
    assert invariant in recording_deletion_upper
for forbidden in (
    "DROP TABLE",
    "DROP COLUMN",
    "DELETE FROM",
    ...
    assert forbidden not in recording_deletion_upper
recording_deletion_service = read(
    "apps/control-plane/src/main/java/io/browsercloud/application/RecordingRetentionDeletionApplicationService.java"
)
for invariant in (
    "FOR UPDATE OF job, recording SKIP LOCKED",
    'nodeHasCapability(job.nodeId(), CAPABILITY, CAPABILITY_VERSION)',
    '"verified-prefix-tombstone-v1"',
    '"DELETE_EXPIRED_OBJECTS"',
):
    assert invariant in recording_deletion_service
for invariant in (
...
agent_action_service = read(
    "apps/control-plane/src/main/java/io/browsercloud/application/AgentActionToolService.java"
)
node_command_proto = read("packages/contracts/proto/node/v1/node_command.proto")
for invariant in (
    "rpc PresignRecordingPlayback(PresignRecordingPlaybackRequest)",
    "message PresignRecordingPlaybackRequest",
    "uint32 segment_limit = 17;",
    "message RecordingPlaybackSegment",
    "message PresignRecordingPlaybackResponse",
    "uint64 next_segment_offset = 12;",
    "rpc DeleteRecordingObjects(DeleteRecordingObjectsRequest)",
    "message DeleteRecordingObjectsRequest",
    "uint64 segment_count = 9;",
    "message DeleteRecordingObjectsResponse",
    "string deletion_proof_hash = 5;",
):
    assert invariant in node_command_proto
routed_command_executor = read(
    "apps/control-plane/src/main/java/io/browsercloud/application/RoutedCoordinatorCommandExecutor.java"
)
assert '"legacy-routed-agent-cancel"' in routed_command_executor
for invariant in (
    ...
    assert invariant in recording_node_agent, (
        f"Browser Node lacks cancellable Agent action rolling invariant: {invariant}"
    )
for invariant in (
    '"agentActionCancellation", "authority-watch-v1"',
    '"AGENT_ACTION_CANCELLATION_UNAVAILABLE"',
):
    assert invariant in agent_action_service, (
        f"Control Plane lacks old-Node Agent action admission gate: {invariant}"
    )
for invariant in (
    "message CancelAgentActionCommand",
    "string session_id = 1;",
    "string task_id = 2;",
    "string reason = 3;",
):
    assert invariant in node_command_proto, (
        f"Agent cancellation protobuf is not additive: {invariant}"
    )
...
outcome_verifier_migration = read(
    "database/migrations/V117__agent_outcome_verifier.sql"
)
outcome_verifier_upper = outcome_verifier_migration.upper()
for invariant in (
    "ADD COLUMN OUTCOME_VERIFICATION_STATUS",
    "'NOT_REQUIRED'",
    "'VERIFYING_OUTCOME'",
    "CREATE TABLE AGENT_OUTCOME_VERIFICATION_JOBS",
    "CREATE TABLE AGENT_OUTCOME_VERIFICATION_EVENTS",
    "OUTCOME-VERIFIER-WORKER/V1",
    "EVIDENCE_HASH",
    "STATE_VERSION",
    "TARGET_REVISION",
    "STATE_HASH",
    ...
    assert invariant in outcome_verifier_upper
for forbidden in (" CAPABILITY_TOKEN ", " RAW_PAGE_BODY ", " SCREENSHOT_BYTES "):
    assert forbidden not in outcome_verifier_upper

outcome_worker_deployment = read(
    "deploy/kubernetes/base/outcome-verifier-worker-deployment.yaml"
)
for invariant in (
    "runtimeClassName: agent-sandbox",
    "automountServiceAccountToken: false",
    ...
    "allowPrivilegeEscalation: false",
    "drop: [ALL]",
    'command: ["python", "/app/outcome_verifier_worker.py"]',
    "--control-plane-token-file=/var/run/browsercloud/identity/token",
    "--model-api-key-file=/var/run/browsercloud/model/api-key",
    "--allowed-model-host=$(MODEL_PROVIDER_ALLOWED_HOST)",
    "--environment=production",
):
    assert invariant in outcome_worker_deployment
for invariant in (
    "/api/v1/agent-outcome-jobs:claim:",
    "operationId: completeAgentOutcomeJob",
    "ClaimAgentOutcomeJobRequest:",
    "AgentOutcomeJobClaim:",
    "const: outcome-verifier-worker/v1",
    "AgentOutcomeVerification:",
):
    assert invariant in openapi
assert "outcomeVerification" not in agent_task_required

expected_outcome_migration = read(
    "database/migrations/V118__agent_expected_outcomes.sql"
)
expected_outcome_upper = expected_outcome_migration.upper()
for invariant in (
    "ADD COLUMN EXPECTED_OUTCOMES",
    "ADD COLUMN OUTCOME_EXPECTED_RESULTS",
    "JSONB NOT NULL DEFAULT '[]'",
    "JSONB_ARRAY_LENGTH(EXPECTED_OUTCOMES) <= 10",
    "JSONB_ARRAY_LENGTH(OUTCOME_EXPECTED_RESULTS) <= 10",
):
    assert invariant in expected_outcome_upper
for forbidden in (" MATCH_VALUE ", " RAW_VALUE ", " ACCESSIBLE_NAME "):
    assert forbidden not in expected_outcome_upper

expected_outcome_request_schema = openapi.split(
    "    ExpectedOutcomeRequest:", 1
)[1].split("    ExpectedOutcomeType:", 1)[0]
assert "writeOnly: true" in expected_outcome_request_schema
expected_outcome_definition_schema = openapi.split(
    "    ExpectedOutcomeDefinition:", 1
)[1].split("    ExpectedOutcomeEvaluation:", 1)[0]
assert "expectedValueHash:" in expected_outcome_definition_schema
assert "matchValue:" not in expected_outcome_definition_schema
assert "expectedOutcomes" not in agent_task_required
...
facts = {
    "schema": "V019-V021 additive,V028,V034,V039-V042,V062-V065,V070,V084-V086,V097 expand-online-index-validate,V098-site-policy-additive-and-validate,V099-recording-manifest-additive-and-validate,V100-secure-debug-notification-admission-function-replace,V101-browser-node-freshness-additive,V102-enterprise-overview-stream-additive,V103-challenge-visual-automation-additive-default-three,V104-agent-autonomous-input-additive-safe-default,V105-agent-human-input-assistance-additive,V106-challenge-human-motion-additive-bounded,V107-session-identity-lock-additive,V108-agent-clipboard-isolated-encrypted,V109-agent-browser-files-additive-direct-stream,V110-agent-browser-screenshot-metadata-and-purpose-grant,V111-agent-browser-javascript-evaluation-additive,V112-agent-clipboard-bridge-additive-purpose-bound,V113-session-soft-delete-additive-audited,V114-browser-state-observed-at-additive,V115-agent-action-attempt-history-additive,V116-agent-task-memory-history-additive,V117-independent-outcome-verifier-additive,V118-agent-expected-outcomes-additive-hash-only,V119-challenge-visual-local-ocr-pii-attestation-additive,V120-worker-queue-notification-triggers-additive,V121-api-idempotency-response-snapshot-additive,V122-challenge-structural-single-click-additive-fail-closed,V123-profile-site-session-health-additive,V124-outcome-semantic-evidence-additive-nullable-for-n-minus-one,V125-opaque-frame-challenge-click-default-off,V126-otp-target-fence-n-minus-one-compatible,V127-recording-playback-purpose-grant-additive,V128-recording-retention-physical-deletion-additive,V129-proxy-business-outcome-learning-additive,V130-proxy-profile-sticky-concurrent-index,V131-proxy-site-challenge-quarantine-additive,V132-proxy-provider-adapter-endpoint-identity-additive,V133-remote-proxy-provider-adapter-type-compatible,online concurrent-index,V029-V033,V035-V038,V043-V060,V066-V068,V071-V076 additive,V077 gameday-expand,V078 gameday-governance-additive,V079 agent-worker-expand,V080 reviewer-worker-expand,V081 agent-human-input-wait-expand,V082 state-resync-budget-additive,V083-state-snapshot-stream-additive,V093-workspace-desktop-actor-quota-additive-default-and-validate,V094-desktop-usage-metering-additive-default-and-validate,V095-profile-export-access-additive-and-validate,V096-profile-warm-tier-journal-additive-and-validate,V061 concurrent-trigram-index,V069 concurrent-agent-summary-index,V070 workspace-overview-stream",
    "protobuf": "unknown-fields-13-16,optional-28-53,browser-transaction-tags-35-37-capability-gated,browser-transaction-site-policy-start-tags-34-38,session-identity-start-tags-39-53,proxy-health-tags-31-34,resource-readback-tags-40-56,remote-desktop-usage-tags-10-12,cold-probe-rpc-request-1-6-response-1-7-capability-gated,extension-tags-15-22,media-slot-tags-16-24,tab-policy-tags-start-23-24-adjust-17-18-event-25-28,extension-background-tags-start-25-adjust-19-20-event-29-30,success-trace-tags-start-26-adjust-21-event-31-32,observer-fps-tags-start-27-adjust-22-event-33-34,recording-tags-start-28-adjust-23-event-35-36,recording-finalized-event-tags-1-14,recording-playback-presign-request-1-17-response-1-12-capability-gated,recording-deletion-request-1-9-response-1-7-capability-gated,screenshot-sampling-tags-start-29-adjust-24-event-37-38,start-minimum-browser-generation-tag-30,evidence-event-tags-1-28,recovery-extension-tag-6,browser-readiness-tags-full-and-diff-11-14,state-snapshot-begin-chunk-commit-additive,resync-request-and-cpu-tags-9-16-17,profile-import-stream-tags-1-10-capability-gated,evidence-presign-tags-request-1-8-response-1-5,profile-export-presign-tags-request-1-5-response-1-8-capability-gated,warm-tier-sync-event-tags-1-13,observer-capture-tags-1-2-and-opaque-frame-tag-14,agent-screenshot-command-tags-1-14-capability-gated,agent-javascript-evaluation-command-tags-1-12-event-tags-1-19-capability-gated,challenge-visual-action-and-failure-and-motion-tags-9-13,challenge-structural-target-tags-14-20,challenge-opaque-frame-fence-tags-21-25,agent-action-sensitive-input-and-batch-tags-14-20,agent-action-primitive-stable-element-tab-and-dialog-tags-12-15,agent-action-advanced-command-tags-21-26-and-primitive-tags-16-22,browser-tab-and-native-dialog-state-full-diff-tags-15-21,interactive-target-structured-fields-8-18,agent-browser-download-full-diff-tags-19-23,agent-file-stage-stream-and-command-additive,agent-action-cancel-command-additive-capability-gated",
    "json": "AUTO-create-without-resource-class,public-resource-template-pricing,new-media-recording-and-application-recovery-fields-optional,recoveryExtensionId-and-approval-metadata-optional,profile-import-and-proxy-binding-additive-endpoints,proxy-provider-routing-metadata,workspace-batch-operation-saved-view-filter-and-metadata-batch-and-agent-summary-and-workspace-overview-and-notification-stream-and-audit-stream-and-enterprise-overview-stream-and-release-freeze-and-validation-worker-and-gameday-worker-and-gameday-governance-and-agent-worker-and-reviewer-worker-and-outcome-verifier-worker-and-human-input-wait-additive-contracts",
    "rolling": "leased-rendezvous-shard-dispatch,durable-routed-coordinator-command-inbox,durable-workspace-batch-command-ledger,isolated-metadata-batch-lease-ledger,isolated-validation-worker-lease-and-claim-token-fencing,isolated-gameday-worker-lease-claim-token-and-recovery-fencing,isolated-agent-worker-lease-claim-token-and-epoch-fencing,isolated-reviewer-worker-lease-claim-token-model-revision-and-plan-hash-fencing,isolated-outcome-verifier-worker-lease-claim-token-model-revision-and-exact-state-evidence-fencing,isolated-vision-worker-lease-claim-token-model-revision-and-screenshot-grant-fencing,proxy-cold-probe-db-lease-and-node-capability,proxy-routing-snapshot-and-fail-closed-selection,migration-target-generation-floor-capability,recording-frame-redaction-capability,migration-target-cleanup-gated-retry,maxUnavailable=0,maxSurge=1,pdb-maxUnavailable=1",
}
