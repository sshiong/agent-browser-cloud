...
for _ in $(seq 1 160); do
  tab_resource_limits="$(docker exec "$postgres_name" psql -U browsercloud -d browsercloud -Atc \
    "select state_collector_budget_percent || ':' || background_tabs_frozen || ':' || new_tabs_blocked || ':' || paused_extension_ids::text
     from browser_placements where session_id='${session_one}'")"
  if [[ "$tab_resource_limits" = '50:true:true:["jdgnleokimdbblcflcfcohbinohmmmlb"]' ]]; then break; fi
  sleep 0.25
done
test "$tab_resource_limits" = '50:true:true:["jdgnleokimdbblcflcfcohbinohmmmlb"]'
...
printf '%s' "$tab_resource_view" | python3 -c \
  'import json,sys; allocation=json.load(sys.stdin)["allocation"]; assert allocation["backgroundTabsFrozen"] is True; assert allocation["newTabsBlocked"] is True; assert allocation["pausedExtensionIds"] == ["jdgnleokimdbblcflcfcohbinohmmmlb"]; assert allocation["stateCollectorBudgetPercent"] == 50'
...
non_cgroup_resource_limits="$(docker exec "$postgres_name" psql -U browsercloud -d browsercloud -Atc \
  "select state_collector_budget_percent || ':' || remote_desktop_bitrate_kbps || ':' || extension_cpu_weight || ':' || media_encoder_slots || ':' || background_tabs_frozen || ':' || new_tabs_blocked
   from browser_placements where session_id='${session_one}'")"
test "$non_cgroup_resource_limits" = "50:0:100:0:true:true"
