...
printf '%s' "$agent_tasks" | python3 -c \
  'import json,sys; tasks=json.load(sys.stdin); assert tasks["total"] == 3; assert len(tasks["items"]) == 3'
...
printf '%s' "$profile_list" | python3 -c \
  'import json,sys; result=json.load(sys.stdin); assert result["total"] == 3; assert {item["profileId"] for item in result["items"]} == {"profile-integration","profile-lifecycle-failover","profile-stopping-failover"}'
