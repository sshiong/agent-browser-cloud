...
EVIDENCE_ARGS=()
for component in control-plane browser-node web-console operator application-adapter validation-worker gameday-worker agent-worker; do
...
python3 "$REPO_ROOT/tools/supply-chain/release_bundle.py" render \
  ...
  --image "gameday-worker=ghcr.io/sshiong/agent-browser-cloud-gameday-worker@$(digest 7)" \
  --image "agent-worker=ghcr.io/sshiong/agent-browser-cloud-agent-worker@$(digest 8)" \
  "${EVIDENCE_ARGS[@]}"
...
if [[ "$image_count" -ne 11 ]]; then
  echo "expected eleven digest-locked workload image references, got $image_count" >&2
  exit 1
fi
