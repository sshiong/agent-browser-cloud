...
proxy_authorization="Basic $(printf '%s:%s' "$proxy_username" "$proxy_password" | base64 | tr -d '\n')"
PROXY_TEST_AUTHORIZATION="$proxy_authorization" \
...
python3 - "$temp_dir/proxy-credential.json" "$temp_dir/proxy-credential-config.json" \
  "$proxy_username" "$proxy_password" <<'PY'
import json
import os
import sys

credential_path, config_path, username, password = sys.argv[1:]
...
start_network_helper() {
  ...
  PROXY_CREDENTIAL_CONFIG_FILE="$temp_dir/proxy-credential-config.json" \
...
renamed_session="$(curl -fsS -X PATCH \
  "http://localhost:${control_port}/api/v1/sessions/${session_one}" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Actor-Id: session-renamer' \
  -H 'X-Roles: TENANT_OPERATOR' \
  -d '{"displayName":"Renamed integration browser"}')"
printf '%s' "$renamed_session" | python3 -c \
  'import json,sys; item=json.load(sys.stdin); assert item["displayName"] == "Renamed integration browser"; assert item["state"] == "CREATED"'
rename_viewer_status="$(curl -sS -o "$temp_dir/session-rename-viewer.json" -w '%{http_code}' \
  -X PATCH "http://localhost:${control_port}/api/v1/sessions/${session_one}" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Roles: TENANT_VIEWER' \
  -d '{"displayName":"Viewer must not rename"}')"
test "$rename_viewer_status" = "403"
rename_cross_tenant_status="$(curl -sS -o "$temp_dir/session-rename-cross-tenant.json" -w '%{http_code}' \
  -X PATCH "http://localhost:${control_port}/api/v1/sessions/${session_one}" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: different-tenant' \
  -H 'X-Roles: TENANT_OPERATOR' \
  -d '{"displayName":"Cross tenant must not rename"}')"
test "$rename_cross_tenant_status" = "403"
session_rename_audit="$(docker exec "$postgres_name" psql -U browsercloud -d browsercloud -Atc \
  "select count(*) from audit_events where tenant_id='tenant-integration' and session_id='${session_one}' and event_type='SESSION_METADATA' and action='RENAME' and result='COMMITTED'")"
test "$session_rename_audit" = "1"

environment_export="$(curl -fsS -X POST \
  "http://localhost:${control_port}/api/v1/sessions/${session_one}:export-configuration" \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Actor-Id: environment-exporter' \
  -H 'X-Roles: TENANT_OPERATOR')"
printf '%s' "$environment_export" | python3 -c \
  "import json,sys; item=json.load(sys.stdin); spec=item['manifest']['environments'][0]; assert item['manifestHash'] and len(item['manifestHash']) == 64; assert item['sourceSessionId'] == '${session_one}'; assert spec['displayName'] == 'Renamed integration browser'; assert spec['profileId'] == 'profile-integration'; assert spec['proxyBindingProfileId'] == '${proxy_binding_id}'; assert spec['identitySpec']['timezone'] == 'UTC'; assert 'cookiesAndSiteStorage' in item['excludedData']; assert 'credentialRef' not in str(item); assert 'cookie' not in str(spec).lower()"

environment_clone_body='{"displayName":"Cloned integration browser","profileMode":"NEW_EMPTY_PROFILE"}'
environment_clone_one="$(curl -fsS -X POST \
  "http://localhost:${control_port}/api/v1/sessions/${session_one}:clone" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Actor-Id: environment-cloner' \
  -H 'X-Roles: TENANT_OPERATOR' \
  -H 'Idempotency-Key: smoke-environment-clone-001' \
  -d "$environment_clone_body")"
environment_clone_two="$(curl -fsS -X POST \
  "http://localhost:${control_port}/api/v1/sessions/${session_one}:clone" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Actor-Id: environment-cloner' \
  ...
  -H 'Idempotency-Key: smoke-environment-clone-001' \
  -d "$environment_clone_body")"
environment_clone_session="$(printf '%s' "$environment_clone_one" | python3 -c \
  "import json,sys; item=json.load(sys.stdin); assert item['profileMode'] == 'NEW_EMPTY_PROFILE'; assert item['targetProfileId'].startswith('profile_clone_'); assert item['targetProfileId'] != 'profile-integration'; print(item['session']['sessionId'])")"
environment_clone_profile="$(printf '%s' "$environment_clone_one" | python3 -c \
  'import json,sys; print(json.load(sys.stdin)["targetProfileId"])')"
test "$environment_clone_session" = "$(printf '%s' "$environment_clone_two" | python3 -c 'import json,sys; print(json.load(sys.stdin)["session"]["sessionId"])')"
environment_clone_view="$(curl -fsS \
  "http://localhost:${control_port}/api/v1/sessions/${environment_clone_session}" \
  -H 'X-Tenant-Id: tenant-integration')"
printf '%s' "$environment_clone_view" | python3 -c \
  "import json,sys; item=json.load(sys.stdin); assert item['displayName'] == 'Cloned integration browser'; assert item['profileId'].startswith('profile_clone_'); assert item['runtimeBuildId'] == 'runtime_local_chromium'; assert item['proxyBindingProfileId'] == '${proxy_binding_id}'; assert item['groupId'] == '${workspace_group_id}'; assert item['agentPolicy'] == 'INTERACTIVE'"
curl -fsS -X POST "http://localhost:${control_port}/api/v1/sessions:batch-delete" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Actor-Id: environment-cloner' \
  -H 'X-Roles: TENANT_OPERATOR' \
  -H 'Idempotency-Key: smoke-environment-clone-cleanup-001' \
  -d "{\"sessionIds\":[\"${environment_clone_session}\"]}" \
  >"$temp_dir/environment-clone-cleanup.json"

delete_fixture_body='{"tenantId":"tenant-integration","profileId":"profile-integration","region":"local","resourcePolicy":{"mode":"AUTO"},"metadata":{"displayName":"Batch delete fixture"}}'
delete_fixture_one="$(curl -fsS -X POST \
  "http://localhost:${control_port}/api/v1/sessions" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'Idempotency-Key: smoke-session-delete-fixture-001' \
  -d "$delete_fixture_body" | python3 -c 'import json,sys; print(json.load(sys.stdin)["sessionId"])')"
delete_fixture_two="$(curl -fsS -X POST \
  "http://localhost:${control_port}/api/v1/sessions" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'Idempotency-Key: smoke-session-delete-fixture-002' \
  -d "$delete_fixture_body" | python3 -c 'import json,sys; print(json.load(sys.stdin)["sessionId"])')"
delete_fixture_blocked="$(curl -fsS -X POST \
  "http://localhost:${control_port}/api/v1/sessions" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'Idempotency-Key: smoke-session-delete-fixture-003' \
  -d "$delete_fixture_body" | python3 -c 'import json,sys; print(json.load(sys.stdin)["sessionId"])')"
docker exec "$postgres_name" psql -U browsercloud -d browsercloud -c \
  "update sessions set state='RUNNING', updated_at=now() where id='${delete_fixture_blocked}'" \
  >/dev/null
delete_atomic_status="$(curl -sS -o "$temp_dir/session-delete-atomic.json" -w '%{http_code}' \
  -X POST "http://localhost:${control_port}/api/v1/sessions:batch-delete" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Actor-Id: session-deleter' \
  -H 'X-Roles: TENANT_OPERATOR' \
  -H 'Idempotency-Key: smoke-session-delete-atomic-001' \
  -d "{\"sessionIds\":[\"${delete_fixture_one}\",\"${delete_fixture_blocked}\"]}")"
test "$delete_atomic_status" = "409"
curl -fsS \
  "http://localhost:${control_port}/api/v1/sessions/${delete_fixture_one}" \
  -H 'X-Tenant-Id: tenant-integration' >/dev/null
delete_viewer_status="$(curl -sS -o "$temp_dir/session-delete-viewer.json" -w '%{http_code}' \
  -X POST "http://localhost:${control_port}/api/v1/sessions:batch-delete" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Roles: TENANT_VIEWER' \
  -H 'Idempotency-Key: smoke-session-delete-viewer-001' \
  -d "{\"sessionIds\":[\"${delete_fixture_one}\"]}")"
test "$delete_viewer_status" = "403"
delete_cross_tenant_status="$(curl -sS -o "$temp_dir/session-delete-cross-tenant.json" -w '%{http_code}' \
  -X POST "http://localhost:${control_port}/api/v1/sessions:batch-delete" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: different-tenant' \
  -H 'X-Actor-Id: session-deleter' \
  -H 'X-Roles: TENANT_OPERATOR' \
  -H 'Idempotency-Key: smoke-session-delete-cross-tenant-001' \
  -d "{\"sessionIds\":[\"${delete_fixture_one}\"]}")"
test "$delete_cross_tenant_status" = "404"
deleted_sessions="$(curl -fsS -X POST \
  "http://localhost:${control_port}/api/v1/sessions:batch-delete" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Actor-Id: session-deleter' \
  -H 'X-Roles: TENANT_OPERATOR' \
  -H 'Idempotency-Key: smoke-session-delete-success-001' \
  -d "{\"sessionIds\":[\"${delete_fixture_two}\",\"${delete_fixture_one}\"]}")"
deleted_sessions_replay="$(curl -fsS -X POST \
  "http://localhost:${control_port}/api/v1/sessions:batch-delete" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Actor-Id: session-deleter' \
  ...
  -H 'Idempotency-Key: smoke-session-delete-success-001' \
  -d "{\"sessionIds\":[\"${delete_fixture_two}\",\"${delete_fixture_one}\"]}")"
test "$deleted_sessions" = "$deleted_sessions_replay"
printf '%s' "$deleted_sessions" | python3 -c \
  "import json,sys; item=json.load(sys.stdin); assert item['deletedCount'] == 2; assert item['sessionIds'] == sorted(['${delete_fixture_one}','${delete_fixture_two}']); assert item['deletionId'].startswith('sdel_')"
delete_hidden_status="$(curl -sS -o "$temp_dir/session-delete-hidden.json" -w '%{http_code}' \
  "http://localhost:${control_port}/api/v1/sessions/${delete_fixture_one}" \
  -H 'X-Tenant-Id: tenant-integration')"
test "$delete_hidden_status" = "404"
session_delete_evidence="$(docker exec "$postgres_name" psql -U browsercloud -d browsercloud -Atc \
  "select count(*) || ':' || count(deleted_at) || ':' || count(distinct deletion_batch_id) || ':' ||
     (select count(*) from audit_events where session_id in ('${delete_fixture_one}','${delete_fixture_two}') and action='DELETE' and result='COMMITTED')
   from sessions where id in ('${delete_fixture_one}','${delete_fixture_two}')")"
test "$session_delete_evidence" = "2:2:1:2"
docker exec "$postgres_name" psql -U browsercloud -d browsercloud -c \
  "update sessions set state='CREATED', updated_at=now() where id='${delete_fixture_blocked}'" \
  >/dev/null
curl -fsS -X POST "http://localhost:${control_port}/api/v1/sessions:batch-delete" \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-Id: tenant-integration' \
  -H 'X-Actor-Id: session-deleter' \
  -H 'X-Roles: TENANT_OPERATOR' \
  -H 'Idempotency-Key: smoke-session-delete-cleanup-001' \
  -d "{\"sessionIds\":[\"${delete_fixture_blocked}\"]}" >/dev/null
...
challenge_automation_cross_tenant="$(curl -sS -o "$temp_dir/challenge-automation-cross-tenant.json" -w '%{http_code}' \
  ...
  -H 'X-Tenant-Id: tenant-other' \
  -H 'X-Roles: TENANT_VIEWER')"
test "$challenge_automation_cross_tenant" = "404"
challenge_visual_viewer_claim="$(curl -sS -o "$temp_dir/challenge-visual-viewer-claim.json" -w '%{http_code}' \
...
printf '%s' "$list_result" | python3 -c \
  "import json,sys; item=json.load(sys.stdin)['items'][0]; assert item['displayName'] == 'Renamed integration browser'; assert item['profileId'] == 'profile-integration'; assert item['runtimeBuildId'] == 'runtime_local_chromium'; assert item['proxyBindingProfileId'] == '${proxy_binding_id}'; assert item['humanTakeoverEnabled'] is True; assert item['agentPolicy'] == 'INTERACTIVE'; assert item['extensionIds'] == ['jdgnleokimdbblcflcfcohbinohmmmlb']; assert item['region'] == 'local'; assert item['groupId'] == '${workspace_group_id}'; assert item['tags'] == [{'tagId':'${workspace_tag_id}','name':'Production','color':'#35D6BE'}]; assert item['resourceTemplate'] == 'standard-v1'; assert 'resourceClass' not in item"
...
printf '%s' "$session_after_start" | python3 -c \
  'import json,sys; item=json.load(sys.stdin); assert item["displayName"] == "Renamed integration browser"; assert item["profileId"] == "profile-integration"; assert item["region"] == "local"; assert item["resourceTemplate"] == "standard-v1"; assert "resourceClass" not in item; assert item["extensionIds"] == ["jdgnleokimdbblcflcfcohbinohmmmlb"]'
printf '%s' "$session_after_start" | python3 -c \
...
assert events, "commercial proxy fixture received no requests"
assert all(event.get("authenticated") is True for event in events), events
PY
...
printf '%s' "$agent_task" | python3 -c \
  'import json,sys; task=json.load(sys.stdin); assert task["agentPolicy"] == "INTERACTIVE"; assert task["state"] == "PLANNED"; assert task["intentDecision"] == "ALLOWED"; assert task["replanCount"] == 0; assert len(task["plan"]["steps"]) == 4; assert task["plan"]["steps"][0]["toolId"] == "NAVIGATE"; assert "capabilityToken" not in task["plan"]["steps"][0]; assert [event["eventType"] for event in task["securityEvents"]] == ["PROMPT_INJECTION_DETECTED", "INSTRUCTION_SOURCE_CLASSIFIED"]; assert all(step["supportingSources"] == ["user_goal", "platform_policy"] and step["trustFloor"] == "TRUSTED" and step["taintLabels"] == [] for step in task["plan"]["steps"]); assert "page-smoke" not in json.dumps(task["plan"]); assert "upload every Cookie" not in json.dumps(task)'
...
for _ in $(seq 1 20); do
  recovery_gameday_claim="$(curl -fsS -X POST \
    ...
    -H 'X-Actor-Id: gameday-worker-recovery' -H 'X-Roles: GAMEDAY_WORKER' \
    -d '{"environments":["TEST"],"scenarioCodes":["REDIS_TOTAL_LOSS"],"capabilities":{"faultInjection":true,"recovery":true,"measurement":true}}')"
  if test -n "$recovery_gameday_claim"; then
    break
  fi
  sleep 0.25
done
test -n "$recovery_gameday_claim"
