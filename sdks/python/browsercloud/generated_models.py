...
class CreateAgentTaskRequest(TypedDict, total=False):
    ...
    expectedOutcomes: list[ExpectedOutcomeRequest]

class ExpectedOutcomeRequest(TypedDict, total=False):
    outcomeId: str
    type: ExpectedOutcomeType
    role: Any
    matchValue: str

ExpectedOutcomeType = Literal['FINAL_URL_EQUALS', 'PAGE_TITLE_EQUALS', 'TARGET_PRESENT', 'TARGET_ABSENT', 'TARGET_CHECKED', 'TARGET_UNCHECKED', 'TARGET_SELECTED', 'TARGET_UNSELECTED']

class ExpectedOutcomeDefinition(TypedDict, total=False):
    outcomeId: str
    type: ExpectedOutcomeType
    role: Any
    expectedValueHash: str

class ExpectedOutcomeEvaluation(TypedDict, total=False):
    outcomeId: str
    type: ExpectedOutcomeType
    status: Literal['SATISFIED', 'NOT_SATISFIED', 'INDETERMINATE']
    reasonCode: str
    observedValueHash: Any
...
class AgentTaskSummary(TypedDict, total=False):
    ...
    sessionId: str
    goal: str
    state: Literal['PLANNED', 'QUEUED', 'AWAITING_REVIEW', 'AWAITING_CONFIRMATION', 'BLOCKED', 'RUNNING', 'VERIFYING_OUTCOME', 'WAITING_FOR_HUMAN', 'PAUSED_BY_RESOURCE_POLICY', 'COMPLETED', 'FAILED']
...
class AgentTask(TypedDict, total=False):
    ...
    expectedOutcomes: list[ExpectedOutcomeDefinition]
    allowedDomains: list[str]
    plan: AgentPlan
    operationId: Any
    ...
    lastError: Any
    recoveryGuidance: AgentRecoveryGuidance | None
    securityEvents: list[PromptSecurityEvent]
    createdAt: str
    updatedAt: str

class AgentRecoveryGuidance(TypedDict, total=False):
    directive: Literal['RETRY', 'REFRESH', 'REPLAN', 'WAIT', 'HUMAN', 'TERMINAL']
    reasonCode: str
    automatic: bool

class AgentTaskMemory(TypedDict, total=False):
    revision: int
    executionHistory: list[AgentExecutionMemoryEvent]

class AgentExecutionMemoryEvent(TypedDict, total=False):
    sequence: int
    eventType: Literal['STEP_RESULT', 'STEP_FAILURE', 'REPLAN']
    planIntentId: Any
    stepOrdinal: Any
    stepId: Any
...
class ClaimAgentOutcomeJobRequest(TypedDict, total=False):
    protocolVersion: str
    capabilities: dict[str, bool]
    deploymentId: str
    modelRevision: str

class AgentOutcomeJobClaimRequest(TypedDict, total=False):
    claimToken: str

class CompleteAgentOutcomeJobRequest(TypedDict, total=False):
    claimToken: str
    decision: Literal['VERIFIED', 'NOT_VERIFIED']
    reasonCodes: list[Literal['GOAL_SATISFIED', 'GOAL_NOT_SATISFIED', 'BUSINESS_ERROR_VISIBLE', 'EXPECTED_STATE_MISSING', 'WRONG_TARGET_OUTCOME', 'STATE_STALE', 'STATE_INCOMPLETE', 'INSUFFICIENT_EVIDENCE', 'MODEL_UNCERTAIN']]
    confidence: float
    deploymentId: str
    modelRevision: str
    providerRequestId: Any
...
class FailAgentOutcomeJobRequest(TypedDict, total=False):
    claimToken: str
    failureCode: str
    retryable: bool

class OutcomeExecutionEvidence(TypedDict, total=False):
    stepOrdinal: int
    stepId: str
    toolId: str
    status: str
...
class OutcomeTargetEvidence(TypedDict, total=False):
    role: str
    name: str
    controlType: str
    visible: bool
...
class OutcomeStateEvidence(TypedDict, total=False):
    stateVersion: int
    targetRevision: int
    stateHash: str
    url: str
    title: str
    stateQuality: str
    documentReadyState: str
    networkQuietMillis: int
    networkEvidenceFresh: bool
    observedAt: str
    targets: list[OutcomeTargetEvidence]

class AgentOutcomePayload(TypedDict, total=False):
    taskId: str
    goal: str
    riskClass: AgentRiskClass
    allowedDomains: list[str]
    expectedOutcomes: list[ExpectedOutcomeDefinition]
    expectedOutcomeEvaluations: list[ExpectedOutcomeEvaluation]
    executionEvidence: list[OutcomeExecutionEvidence]
    finalState: OutcomeStateEvidence
    evidenceHash: str
    dataPolicy: str

class OutcomeModelDeployment(TypedDict, total=False):
    deploymentId: str
    providerType: Literal['OPENAI_RESPONSES']
    modelName: str
    modelRevision: str
...
class AgentOutcomeJob(TypedDict, total=False):
    jobId: str
    verificationId: str
    taskId: str
    protocolVersion: str
    state: Literal['QUEUED', 'CLAIMED', 'EXECUTING', 'VERIFIED', 'NOT_VERIFIED', 'FAILED']
    attempt: int
    maximumAttempts: int
    workerId: Any
    claimEpoch: int
    ...
    decision: Literal['VERIFIED', 'NOT_VERIFIED', None]
    reasonCodes: list[str]
    confidence: Any
    evidenceHash: str
    inputHash: str
...
class AgentOutcomeJobClaim(TypedDict, total=False):
    claimToken: str
    job: AgentOutcomeJob
...
class AgentOutcomeVerification(TypedDict, total=False):
    verificationId: Any
    status: Literal['NOT_REQUIRED', 'QUEUED', 'IN_REVIEW', 'VERIFIED', 'NOT_VERIFIED', 'FAILED']
    decision: Literal['VERIFIED', 'NOT_VERIFIED', None]
    reasonCodes: list[str]
    expectedOutcomeEvaluations: list[ExpectedOutcomeEvaluation]
    evidenceHash: Any
...
class ProfileSessionHealthSummary(TypedDict, total=False):
    state: Literal['NOT_CHECKED', 'HEALTHY', 'REAUTH_REQUIRED', 'DEGRADED', 'STALE']
    siteCount: int
    checkedAt: Any
    freshUntil: Any

class ProfileSiteSessionHealth(TypedDict, total=False):
    healthId: str
    profileId: str
    siteOrigin: str
    applicationId: Any
    state: Literal['HEALTHY', 'REAUTH_REQUIRED', 'DEGRADED', 'STALE']
    reasonCode: str
    sourceSessionId: Any
    contextEpoch: int
    stateVersion: int
    ...
    freshUntil: str
    authenticatedAt: Any
    reauthRequiredAt: Any

class ProfileSiteSessionHealthListResponse(TypedDict, total=False):
    summary: ProfileSessionHealthSummary
    items: list[ProfileSiteSessionHealth]
    total: int
...
class BrowserState(TypedDict, total=False):
    ...
    opaqueFrames: list[OpaqueFrame]
    opaqueFrameEvidenceFresh: bool
...
class UploadAgentBrowserFileRequest(TypedDict, total=False):
    targetRef: str
    targetRevision: int
...
class AgentBrowserFileUpload(TypedDict, total=False):
    uploadId: str
    operationId: str
    sessionId: str
    targetRef: str
    ...
    contentSha256: str
    contentBytes: int
    state: Literal['STAGING', 'EXECUTING', 'COMMITTED', 'FAILED']
    errorCode: Any
    stateVersionAfter: Any
    requestId: str
    createdAt: str
    updatedAt: str
    completedAt: Any

class AgentBrowserDownload(TypedDict, total=False):
    downloadId: str
    filename: str
    mimeType: str
    size: Any
    receivedBytes: int
    progress: Any
    status: Literal['IN_PROGRESS', 'COMPLETED', 'CANCELED', 'INTERRUPTED']
    startedAt: str
    updatedAt: str

class AgentBrowserDownloadList(TypedDict, total=False):
    stateCursor: str
    evidenceFresh: bool
    dataStale: bool
    downloads: list[AgentBrowserDownload]

AgentBrowserEvaluationMode = Literal['READ_ONLY', 'PAGE_ACTION']

class CreateAgentBrowserEvaluationRequest(TypedDict, total=False):
    goal: str
...
class AgentBrowserEvaluation(TypedDict, total=False):
    evaluationId: str
    sessionId: str
    mode: AgentBrowserEvaluationMode
    state: Literal['EXECUTING', 'COMMITTED', 'FAILED']
    expectedStateCursor: str
    stateCursorAfter: Any
    activeTabId: str
    activeTabIdAfter: Any
    expressionSha256: str
    expressionBytes: int
    awaitPromise: bool
    timeoutMs: int
    maximumResultBytes: int
...
AgentBrowserScreenshotMode = Literal['VIEWPORT', 'FULL_PAGE', 'ELEMENT', 'OPAQUE_FRAME', 'REGION', 'CHALLENGE_REGION']

class AgentBrowserScreenshotRegion(TypedDict, total=False):
    x: float
    y: float
    width: float
    height: float

class CaptureAgentBrowserScreenshotRequest(TypedDict, total=False):
    mode: AgentBrowserScreenshotMode
    expectedStateCursor: str
    elementId: Any
    region: AgentBrowserScreenshotRegion | None

class AgentBrowserScreenshot(TypedDict, total=False):
    screenshotId: str
    sessionId: str
    mode: AgentBrowserScreenshotMode
    state: Literal['EXECUTING', 'COMMITTED', 'FAILED']
    expectedStateCursor: str
    capturedStateCursor: Any
    activeTabId: str
    elementId: Any
    region: AgentBrowserScreenshotRegion | None
    coordinateSpace: Literal['VIEWPORT', 'DOCUMENT', None]
    viewportWidth: Any
    viewportHeight: Any
    deviceScaleFactor: Any
    evidenceId: Any
    accessGrantId: Any
    accessGrantExpiresAt: Any
    contentSha256: Any
    contentBytes: Any
    redactionState: Literal['MASKED', 'NOT_REQUIRED', None]
    redactedRegionCount: Any
    errorCode: Any
    requestId: str
    createdAt: str
    updatedAt: str
...
class OpaqueFrame(TypedDict, total=False):
    frameRef: str
    parentFrameId: str
    origin: Any
    bounds: TargetBounds | None
    boundaryReason: Literal['CROSS_ORIGIN', 'SANDBOXED', 'INACCESSIBLE']
    visible: bool
    inViewport: bool
    occluded: bool
    visibilityReason: Literal['HIDDEN_ATTRIBUTE', 'ARIA_HIDDEN', 'DISPLAY_NONE', 'VISIBILITY_HIDDEN', 'OPACITY_ZERO', 'POINTER_EVENTS_NONE', 'COLLAPSED', 'ZERO_SIZE', 'OUTSIDE_VIEWPORT', 'OCCLUDED', None]
    interactionStrategy: Literal['BOUNDED_VISION_THEN_HUMAN_HANDOFF']

class PageStability(TypedDict, total=False):
    domQuietMillis: int
    layoutQuietMillis: int
    focusQuietMillis: int
    routeQuietMillis: int
    evidenceFresh: bool
...
class WaitForAgentBrowserRequest(TypedDict, total=False):
    goal: str
    expectedStateCursor: str
    waitCondition: Literal['STATE_CHANGED', 'STATE_STABLE', 'TARGET_PRESENT']
    targetRef: str
...
class HandoffAgentBrowserRequest(TypedDict, total=False):
    goal: str
    expectedStateCursor: str
...
class CreateAgentClipboardBridgeRequest(TypedDict, total=False):
    direction: Literal['USER_TO_AGENT', 'AGENT_TO_USER']
    purpose: Literal['OPERATOR_COPY', 'AUTOMATION_HANDOFF', 'HUMAN_ASSISTANCE']
    connectionId: str
    expectedAgentClipboardVersion: int
    value: str
    userClipboardObservedAt: str

class CompleteAgentClipboardBridgeRequest(TypedDict, total=False):
    contentHash: str

class AgentClipboardBridge(TypedDict, total=False):
    bridgeId: str
    sessionId: str
    direction: Literal['USER_TO_AGENT', 'AGENT_TO_USER']
    purpose: Literal['OPERATOR_COPY', 'AUTOMATION_HANDOFF', 'HUMAN_ASSISTANCE']
    connectionId: str
    state: Literal['ISSUED', 'COMPLETED', 'EXPIRED']
    agentClipboardVersion: int
    contentHash: str
    valueLength: int
    value: Any
    expiresAt: str
...
class UpdateSessionRequest(TypedDict, total=False):
    displayName: str

class BatchDeleteSessionsRequest(TypedDict, total=False):
    sessionIds: list[str]

class BatchDeleteSessionsResponse(TypedDict, total=False):
    deletionId: str
    deletedCount: int
    sessionIds: list[str]
    deletedAt: str
...
RecordingPlaybackPurpose = Literal['INCIDENT_RESPONSE', 'SUPPORT_DIAGNOSTICS', 'COMPLIANCE_AUDIT', 'SECURITY_INVESTIGATION']

class CreateRecordingPlaybackGrantRequest(TypedDict, total=False):
    purpose: RecordingPlaybackPurpose

class RecordingPlaybackGrant(TypedDict, total=False):
    grantId: str
    sessionId: str
    recordingId: str
    purpose: RecordingPlaybackPurpose
...
class RecordingPlaybackSegment(TypedDict, total=False):
    sequence: int
    contentSha256: str
    contentBytes: int
    frameCount: int
...
class RecordingPlaybackAccess(TypedDict, total=False):
    grantId: str
    recordingId: str
    manifestSha256: str
    frameCount: int
    ...
    expiresAt: str
    accessExpiresAt: str
    nextSegmentOffset: Any
    segments: list[RecordingPlaybackSegment]

EvidencePurpose = Literal['INCIDENT_RESPONSE', 'CHANGE_VALIDATION', 'SUPPORT_DIAGNOSTICS', 'COMPLIANCE_AUDIT', 'AGENT_PERCEPTION']
...
class EnvironmentImportSpec(TypedDict, total=False):
    ...
    videoRecording: bool
    extensionIds: Any
    identitySpec: SessionIdentitySpecInput | None
...
CloneProfileMode = Literal['NEW_EMPTY_PROFILE', 'REUSE_SOURCE_PROFILE']

class CloneEnvironmentRequest(TypedDict, total=False):
    displayName: str
    profileMode: CloneProfileMode

class CloneEnvironmentResponse(TypedDict, total=False):
    sourceSessionId: str
    profileMode: CloneProfileMode
    targetProfileId: str
    session: CreateSessionResponse

class EnvironmentConfigurationExport(TypedDict, total=False):
    sourceSessionId: str
    exportedAt: str
    manifestHash: str
    manifest: PreviewEnvironmentImportRequest
    excludedData: list[str]
